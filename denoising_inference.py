...
from inference.data.denoise_dataloader import denoise_dataloader
...
def main():
    ...
    parser.add_argument('--mean', type=str, default=0)
    parser.add_argument('--std', type=str, default=30)
    args = parser.parse_args()

    DEVICE = torch.device("cuda:{}".format(args.gpu) if (torch.cuda.is_available() and args.gpu != 'None') else "cpu")

    save_dir = './inference/results/denoising/'
    os.makedirs(save_dir, exist_ok=True)

    config_path = './inference/denoising_inference_config.yaml'
    config = OmegaConf.load(config_path)

    model = InferenceModel(**config).to(DEVICE)
    model.eval()

    valid_data = denoise_dataloader(**config.data_config, mean=int(args.mean), std=int(args.std), valid=True)
    ...
    with torch.no_grad():
        for in_, target_, img_path in tqdm(valid_data['loader']):
            ...
            # split into patches
            patches, orig_shape, padded_shape, unfolded_shape = img2patch(in_, k=64, s=32)
            # model forward
            out_patches = model(patches)
            # aggregate to image
            out_ = patch2img(out_patches, 64, 32, orig_shape, padded_shape, unfolded_shape)
            # scale to [0.0, 255.0]
