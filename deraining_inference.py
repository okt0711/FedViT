...
from inference.data.derain_dataloader import derain_dataloader
...
def main():
    ...
    save_dir = './inference/results/deraining/'
    os.makedirs(save_dir, exist_ok=True)

    config_path = './inference/deraining_inference_config.yaml'
    config = OmegaConf.load(config_path)

    model = InferenceModel(**config).to(DEVICE)
    model.eval()

    valid_data = derain_dataloader(**config.data_config, valid=True)
